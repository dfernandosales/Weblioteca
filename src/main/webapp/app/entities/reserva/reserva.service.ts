...
export class ReservaService {
    ...
    protected convertDateFromClient(reserva: IReserva): IReserva {
        const copy: IReserva = Object.assign({}, reserva, {
            dataReseva: reserva.dataReseva != null && reserva.dataReseva.isValid() ? reserva.dataReseva.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
        if (res.body) {
            res.body.dataReseva = res.body.dataReseva != null ? moment(res.body.dataReseva) : null;
        }
        return res;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((reserva: IReserva) => {
                reserva.dataReseva = reserva.dataReseva != null ? moment(reserva.dataReseva) : null;
            });
        }
        return res;
    }
